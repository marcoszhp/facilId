...
beforeEach(async()=>{chip=(await request(app).post('/api/emissao').set('X-Admin-Token',adminToken).send({cpf:'12345678900',nome:'Maria Silva',idade:72}).expect(201)).body;});
...
test('reemissão substitui o cartão anterior',async()=>{await request(app).post('/api/emissao').set('X-Admin-Token',adminToken).send({cpf:chip.cpf,nome:'Maria Atualizada',idade:73});expect((await login(chip)).status).toBe(401);});
test('dados e chaves sobrevivem à reinicialização',async()=>{const restarted=createApp(dir,secret,adminToken);expect((await request(restarted).post('/api/autenticar-nfc').send({cpfDigitado:chip.cpf,dadosChip:chip})).status).toBe(200);});
test.each([{cpf:'abc',nome:'Maria',idade:72},{cpf:'12345678900',nome:'',idade:72},{cpf:'12345678900',nome:'Maria',idade:-1},{cpf:'12345678900',nome:'Maria',idade:'72'},{cpf:'12345678900',nome:'Maria',idade:72,admin:true}])('cadastro inválido é recusado: caso %#',async(data)=>{expect((await request(app).post('/api/emissao').set('X-Admin-Token',adminToken).send(data)).status).toBe(400);});
