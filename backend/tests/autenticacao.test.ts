...
const adminToken='chave-administrativa-exclusiva-de-testes-32-caracteres';
let dir:string,app:ReturnType<typeof createApp>,chip:any;
beforeAll(()=>{dir=mkdtempSync(path.join(tmpdir(),'facilid-tests-'));app=createApp(dir,secret,adminToken);});
afterAll(()=>rmSync(dir,{recursive:true,force:true}));
beforeEach(async()=>{chip=(await request(app).post('/api/emissao').set('X-Admin-Token',adminToken).send({...coletaTeste,cpf:'12345678900',nome:'Maria Silva',idade:72}).expect(201)).body;});
const login=(dadosChip:any,cpfDigitado='12345678900')=>loginCompleto(app,dadosChip,cpfDigitado);
test('emissão → login → perfil protegido; JWT expira em 15 minutos',async()=>{
  ...
  expect(token.emissaoId).toBe(chip.emissaoId);expect(result.body.expiraEm).toBe(token.exp!*1000);
...
test('reemissão substitui o cartão anterior',async()=>{await request(app).post('/api/emissao').set('X-Admin-Token',adminToken).send({...coletaTeste,cpf:chip.cpf,nome:'Maria Atualizada',idade:73});expect((await login(chip)).status).toBe(401);});
test('dados e chaves sobrevivem à reinicialização',async()=>{const restarted=createApp(dir,secret,adminToken);expect((await loginCompleto(restarted,chip,chip.cpf)).status).toBe(200);});
test.each([{cpf:'abc',nome:'Maria',idade:72},{cpf:'12345678900',nome:'',idade:72},{cpf:'12345678900',nome:'Maria',idade:-1},{cpf:'12345678900',nome:'Maria',idade:'72'},{cpf:'12345678900',nome:'Maria',idade:72,admin:true}])('cadastro inválido é recusado: caso %#',async(data)=>{expect((await request(app).post('/api/emissao').set('X-Admin-Token',adminToken).send({...coletaTeste,...data})).status).toBe(400);});
test.each([{}, {cpfDigitado:'12345678900',dadosChip:null},{cpfDigitado:'12345678900',dadosChip:{}}])('login malformado retorna 401: caso %#',async(data)=>{expect((await request(app).post('/api/autenticar-nfc').send(data)).status).toBe(401);});
test('JSON inválido e corpo excessivo têm erros controlados',async()=>{
  expect((await request(app).post('/api/emissao').set('X-Admin-Token',adminToken).set('Content-Type','application/json').send('{')).status).toBe(400);
  expect((await request(app).post('/api/emissao').set('X-Admin-Token',adminToken).send({data:'x'.repeat(80000)})).status).toBe(413);
});
...
test('documentação pública e listagem administrativa sem credenciais assinadas',async()=>{
  ...
  const lista=await request(app).get('/api/usuarios').set('X-Admin-Token',adminToken);expect(lista.body.length).toBeGreaterThan(0);
  expect(lista.body[0]).not.toHaveProperty('assinatura_digital_orgao');
  expect((await request(app).get('/api/usuarios?foo=1').set('X-Admin-Token',adminToken)).status).toBe(400);
});
