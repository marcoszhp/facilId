...
export function emitirPessoa(dados:z.infer<typeof cadastroSchema>, assinatura:ReturnType<typeof assinaturaService>, repo:UsuariosRepository) {
  ...
  const chip = assinatura.assinar({...dados,versao:2,emissaoId:randomUUID(),rosto_hash:hash('rosto'),digital_template:hash('digital'),assinatura_svg:'M10 80 Q 52 10 100 80 T 190 80'});
