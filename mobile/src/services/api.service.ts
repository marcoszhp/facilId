...
export function criarApi(url:string) {
  ...
  return {
    ...
    async emitir(p:DadosEmissao,token:string,signal?:AbortSignal) {return (await http.post<Chip>('/api/emissao',{...p,cpf:normalizarCpf(p.cpf)},admin(token,signal))).data;},
    ...
    async entrar(cpf:string,texto:string,signal?:AbortSignal) {const dadosChip=lerIdentidade(texto,cpf);return (await http.post<Desafio>('/api/autenticar-nfc',{cpfDigitado:normalizarCpf(cpf),dadosChip},{signal})).data;},
    async confirmar(dados:Confirmacao,signal?:AbortSignal) {return (await http.post<Sessao&{credencialDispositivo?:string}>('/api/autenticar-confirmar',dados,{signal})).data;},
